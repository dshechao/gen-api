...
func FilterForApiDoc(c *revel.Controller, fc []revel.Filter) {
	if record, _ := revel.Config.Bool("gen.record"); !record {
		fc[0](c, fc[1:])
		return
	}
	...
	if !gen.IsStatusCodeValid(c.Response.Status) {
		return
	}
	...
	for k, v := range rawrequest.Header {
		isCommon := false
		for _, hk := range gen.CommonHeaders {
			if k == hk {
				isCommon = true
	...
	go gen.GenerateHtml(&htmlValues)
}
