/*
 * This is the main core of the gen package
 */
package gen
...
func GenerateHtml(apiCall *models.ApiCall) {
	...
	if shouldAddPathSpec {
		apiSpec := models.ApiSpec{
			...
			CurrentApiComment: apiCall.CurrentApiComment,
			CurrentApiName:    apiCall.CurrentApiName,
		}
		apiCall.Id = count
		count += 1
		deleteCommonHeaders(apiCall)
...
func generateHtml() {
	...
	spec := spec.ApiSpecs
	t.Execute(homeWriter, map[string]interface{}{"array": spec,
